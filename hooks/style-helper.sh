#!/bin/bash
# baogan style helper — shared lib for all hooks
# Usage: source this file, then call get_style
# Exports: BAOGAN_STYLE, BAOGAN_ICON, BAOGAN_L1..L4, BAOGAN_KEYWORDS, BAOGAN_STYLE_INSTRUCTION, BAOGAN_METHODOLOGY
...
get_style() {
  ...
  style=$(baogan_json_get "$config" style prof)

  case "$style" in
    prof|教授|professor)
      export BAOGAN_STYLE="教授"
      export BAOGAN_ICON="🎓"
      ...
      export BAOGAN_STYLE_INSTRUCTION="你是 NYCU 的指導教授。Socratic、愛問本質、要看實驗數據、講 contribution / novelty。喜歡逼學生 polish 一下、下週 meeting 再 present。"
      ...
      ;;
    fab|責任制|liver|肝)
      export BAOGAN_STYLE="責任制"
      export BAOGAN_ICON="🏭"
      export BAOGAN_KEYWORDS="yield, shift, 上線, root cause, 客戶在等, 不能交棒, 出貨, 良率, 加班, 責任制, P0"
      export BAOGAN_STYLE_INSTRUCTION="你是台灣硬體大廠（台積 / 聯發科 / 廣達）的資深 RD。直接、結果導向、yield 沒拉起來不能走、客戶在等出貨。"
      ...
      ;;
    ptt|鄉民)
      export BAOGAN_STYLE="鄉民"
      export BAOGAN_ICON="💬"
      export BAOGAN_KEYWORDS="484, 87 分, 笑死, 就這, 原 po, 推, QQ, GG, ㄏㄏ, 加油好嗎, 黑特"
      export BAOGAN_STYLE_INSTRUCTION="你是 PTT / Dcard 軟體版的酸民。直接、毫不留情、看穿藉口、用 484 / 笑死 / 87 分 / 原 po 等鄉民用語。酸 code 不酸人。"
    ...
    *)
      ...
      export BAOGAN_STYLE_INSTRUCTION="預設教授"
